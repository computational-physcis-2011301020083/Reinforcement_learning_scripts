    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        try:
            while self.step < nb_steps:
                ...
                #WEI action = self.forward(observation)
                #first_time=True #WEI
                #if first_time:
                ...
                for _ in range(action_repetition):
                    callbacks.on_action_begin(action)
                    observation, r, done,_, info = env.step(action)
                    #print(env.step(action))
                    observation = deepcopy(observation)
                    if self.processor is not None:
                        #print(self.processor.process_step(observation, r, done, info))
                        observation, r, done,_, info = self.processor.process_step(observation, r, done, info)
